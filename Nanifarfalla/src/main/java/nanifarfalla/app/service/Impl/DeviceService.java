...
import nanifarfalla.app.repository.DeviceMetadataRepository;
import nanifarfalla.app.service.IDataService;
...
@Service
public class DeviceService implements IDataService {
	 ...
	 * DatabaseReader databaseReader, Parser parser, JavaMailSender mailSender,
	 * MessageSource messages) throws IOException , GeoIp2Exception {
	 * 
	 * this.deviceMetadataRepository = deviceMetadataRepository; this.databaseReader
	 * = databaseReader; this.parser = parser; this.mailSender = mailSender;
	...
	public void verifyDevice(Usuario user, HttpServletRequest request) throws IOException, GeoIp2Exception {
		...
		if (Objects.isNull(existingDevice)) {
			...
			deviceMetadata.setUserId(user.getCodigo_usuario());
	...
	/**
	 * Get the message with a provided locale. If not found, try default
	 * {@link Locale#ENGLISH}.
	 *
