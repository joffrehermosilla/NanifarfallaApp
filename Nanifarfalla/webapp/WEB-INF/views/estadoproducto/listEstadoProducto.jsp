...
<title>Administración de Estado de Producto</title>
...
<body>
	...
	<div class="container theme-showcase" role="main">
		<h3>Listado de Estados de Producto</h3>
		<c:if test="${mensaje!=null }">
			<div class='alert alert-success' role="alert">${ mensaje}</div>
		</c:if>
		<a href="${urlCreate}" class="btn btn-success" role="button"
			title="Nuevo Estado Producto">Nuevo</a><br> <br>
		<div class="table-responsive">
			<table class="table table-hover table-striped table-bordered">
				...
				<c:forEach var="estadoproductoy" items="${estadoproductoy}">
					<tr>
						<td>Tabla Estado Producto</td>
						<td>${estadoproductoy.codigo_estadoproducto}</td>
						<td>${estadoproductoy.nombre_estado_producto}</td>
						<td>${estadoproductoy.claveApi}</td>
						
						<td><fmt:formatDate pattern="dd-MM-yyyy"
								value="${estadoproductoy.version}" /></td>
