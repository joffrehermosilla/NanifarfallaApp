...
ï»¿/**
	 * jQuery.placeholder - Placeholder plugin for input fields Written by Blair
	 * Mitchelmore (blair DOT mitchelmore AT gmail DOT com) Licensed under the
	 * WTFPL (http://sam.zoy.org/wtfpl/). Date: 2008/10/14
	 * 
...
/**
 * TextAreaExpander plugin for jQuery v1.0 Expands or contracts a textarea
 * height depending on the quatity of content entered by the user in the box.
 * 
 ...
 * 
 * Copyright (c) 2006 JÃ¶rn Zaefferer and Brandon Aaron (brandon.aaron@gmail.com ||
...
/**
 * Copyright (c) 2008 Kelvin Luck (http://www.kelvinluck.com/) Dual licensed
 * under the MIT (http://www.opensource.org/licenses/mit-license.php) and GPL
 * (http://www.opensource.org/licenses/gpl-license.php) licenses. . $Id:
 ...
 * 
 * @param Object
 *            s (optional) Customize your calendars.
 * @option Number month The month to render (NOTE that months are zero based).
 *         Default is today's month.
 * @option Number year The year to render. Default is today's year.
 * @option Function renderCallback A reference to a function that is called as
 *         each cell is rendered and which can add classes and event listeners
 *         to the created nodes. Default is no callback.
 * @option Number showHeader Whether or not to show the header row, possible
 *         values are: $.dpConst.SHOW_HEADER_NONE (no header),
 *         $.dpConst.SHOW_HEADER_SHORT (first letter of each day) and
 *         $.dpConst.SHOW_HEADER_LONG (full name of each day). Default is
 *         $.dpConst.SHOW_HEADER_SHORT.
 * @option String hoverClass The class to attach to each cell when you hover
 *         over it (to allow you to use hover effects in IE6 which doesn't
 *         support the :hover pseudo-class on elements other than links).
 *         Default is dp-hover. Pass false if you don't want a hover class.
 * @type jQuery
 * @name renderCalendar
 ...
 *       of calendar-me.
 * 
 * @example var testCallback = function($td, thisDate, month, year) { if
 *          ($td.is('.current-month') && thisDate.getDay() == 4) { var d =
 *          thisDate.getDate(); $td.bind( 'click', function() { alert('You
 *          clicked on ' + d + '/' + (Number(month)+1) + '/' + year); }
 *          ).addClass('thursday'); } else if (thisDate.getDay() == 5) {
 *          $td.html('Friday the ' + $td.html() + 'th'); } }
 *          $('#calendar-me').renderCalendar({month:0, year:2007,
 *          renderCallback:testCallback});
 * 
 * @desc Renders a calendar displaying January 2007 into the element with an id
 *       of calendar-me. Every Thursday in the current month has a class of
 *       "thursday" applied to it, is clickable and shows an alert when clicked.
 *       Every Friday on the calendar has the number inside replaced with text.
 */
		renderCalendar  :   function(s)
		{
			...
			while (w++<weeksToDraw) {
				...
				for (var i=0; i<7; i++) {
					...
					// addDays(1) fails in some locales due to daylight savings.
					// See issue 39.
					// currentDate.addDays(1);
					// set the time to midday to avoid any weird timezone
					// issues??
 ...
 * 
 * The matched element will receive a few custom events with the following
 * signatures:
 * 
 ...
 * object wrapped around the TD that was clicked on and status is whether the
 * date was selected (true) or deselected (false)
 * 
 * dpClosed(event, selected) Triggered when the date picker is closed. event is
 * a reference to the event and selected is an Array containing Date objects.
 * 
 * dpMonthChanged(event, displayedMonth, displayedYear) Triggered when the month
 * of the popped up calendar is changed. event is a reference to the event,
 * displayedMonth is the number of the month now displayed (zero based) and
 * displayedYear is the year of the month.
 * 
 * dpDisplayed(event, $datePickerDiv) Triggered when the date picker is created.
 * $datePickerDiv is the div containing the date picker. Use this event to add
 * custom content/ listeners to the popped up date picker.
 * 
 * @param Object
 *            s (optional) Customize your date pickers.
 ...
 * @option Number year The year to render when the date picker is opened.
 *         Default is today's year.
 ...
 * @option Boolean inline Whether to create the datePicker as inline (e.g.
 *         always on the page) or as a model popup. Default is false (== modal
 *         popup)
 * @option Boolean createButton Whether to create a .dp-choose-date anchor
 *         directly after the matched element which when clicked will trigger
 *         the showing of the date picker. Default is true.
 * @option Boolean showYearNavigation Whether to display buttons which allow the
 *         user to navigate through the months a year at a time. Default is
 *         true.
 * @option Boolean closeOnSelect Whether to close the date picker when a date is
 *         selected. Default is true.
 * @option Boolean displayClose Whether to create a "Close" button within the
 *         date picker popup. Default is false.
 * @option Boolean selectMultiple Whether a user should be able to select
 *         multiple dates with this date picker. Default is false.
 * @option Number numSelectable The maximum number of dates that can be selected
 *         where selectMultiple is true. Default is a very high number.
 * @option Boolean clickInput If the matched element is an input type="text" and
 *         this option is true then clicking on the input will cause the date
 *         picker to appear.
 * @option Boolean rememberViewedMonth Whether the datePicker should remember
 *         the last viewed month and open on it. If false then the date picker
 *         will always open with the month for the first selected date visible.
 * @option Boolean selectWeek Whether to select a complete week at a time...
 * @option Number verticalPosition The vertical alignment of the popped up date
 *         picker to the matched element. One of $.dpConst.POS_TOP and
 *         $.dpConst.POS_BOTTOM. Default is $.dpConst.POS_TOP.
 * @option Number horizontalPosition The horizontal alignment of the popped up
 *         date picker to the matched element. One of $.dpConst.POS_LEFT and
 *         $.dpConst.POS_RIGHT.
 * @option Number verticalOffset The number of pixels offset from the defined
 *         verticalPosition of this date picker that it should pop up in.
 *         Default in 0.
 * @option Number horizontalOffset The number of pixels offset from the defined
 *         horizontalPosition of this date picker that it should pop up in.
 *         Default in 0.
 * @option (Function|Array) renderCallback A reference to a function (or an
 *         array of separate functions) that is called as each cell is rendered
 *         and which can add classes and event listeners to the created nodes.
 *         Each callback function will receive four arguments; a jquery object
 *         wrapping the created TD, a Date object containing the date this TD
 *         represents, a number giving the currently rendered month and a number
 *         giving the currently rendered year. Default is no callback.
 * @option String hoverClass The class to attach to each cell when you hover
 *         over it (to allow you to use hover effects in IE6 which doesn't
 *         support the :hover pseudo-class on elements other than links).
 *         Default is dp-hover. Pass false if you don't want a hover class.
 * @option String autoFocusNextInput Whether focus should be passed onto the
 *         next input in the form (true) or remain on this input (false) when a
 *         date is selected and the calendar closes
 * @type jQuery
 * @name datePicker
 * @cat plugins/datePicker
 * @author Kelvin Luck (http://www.kelvinluck.com/)
 ...
 * @desc Creates a date picker button next to all matched input elements. When
 *       the button is clicked on the value of the selected date will be placed
 *       in the corresponding input (formatted according to Date.format).
 * 
 ...
 */
		...
		{			
			...
			// initialise the date picker controller with the relevant
			// settings...
			s = $.extend({}, $.fn.datePicker.defaults, s);
					...
					if (!alreadyExists && $this.is(':text')) {
						...
						if (s.clickInput) {
							$this.bind(
								...
								function()
								{
									// The change event doesn't happen until the
									// input loses focus so we need to manually
									// trigger it...
									$this.trigger('change');
									$this.dpDisplay();
								}
 ...
 * 
 * @param Boolean
 *            s Whether to disable (true) or enable (false) this datePicker
 * @type jQuery
 * @name dpSetDisabled
 * @cat plugins/datePicker
 * @author Kelvin Luck (http://www.kelvinluck.com/)
 ...
 * @desc Prevents this date picker from displaying and adds a class of
 *       dp-disabled to it (and it's associated button if it has one) for
 *       styling purposes. If the matched element is an input field then it will
 *       also set the disabled attribute to stop people directly editing the
 *       field.
 */
...
/**
 * Updates the first selectable date for any date pickers on any matched
 * elements.
 * 
 * @param String|Date
 *            d A Date object or string representing the first selectable date
 *            (formatted according to Date.format).
 * @type jQuery
 * @name dpSetStartDate
 * @cat plugins/datePicker
 * @author Kelvin Luck (http://www.kelvinluck.com/)
 ...
 * @desc Creates a date picker associated with all elements with a class of
 *       "date-picker" then sets the first selectable date for each of these to
 *       the first day of the millenium.
 */
...
/**
 * Updates the last selectable date for any date pickers on any matched
 * elements.
 * 
 * @param String|Date
 *            d A Date object or string representing the last selectable date
 *            (formatted according to Date.format).
 * @type jQuery
 * @name dpSetEndDate
 * @cat plugins/datePicker
 ...
 * @desc Creates a date picker associated with all elements with a class of
 *       "date-picker" then sets the last selectable date for each of these to
 *       the first Janurary 2010.
 */
...
/**
 * Gets a list of Dates currently selected by this datePicker. This will be an
 * empty array if no dates are currently selected or NULL if there is no
 * datePicker associated with the matched element.
 * 
 * @type Array
 * @name dpGetSelected
 * @cat plugins/datePicker
 ...
 * is only useful on date pickers where selectMultiple==true. Selecting will
 * only work if the passed date is within the startDate and endDate boundries
 * for a given date picker.
 * 
 * @param String|Date
 *            d A Date object or string representing the date you want to select
 *            (formatted according to Date.format).
 * @param Boolean
 *            v Whether you want to select (true) or deselect (false) this date.
 *            Optional - default = true.
 * @param Boolean
 *            m Whether you want the date picker to open up on the month of this
 *            date when it is next opened. Optional - default = true.
 * @param Boolean
 *            e Whether you want the date picker to dispatch events related to
 *            this change of selection. Optional - default = true.
 * @type jQuery
 * @name dpSetSelected
 * @cat plugins/datePicker
 * @author Kelvin Luck (http://www.kelvinluck.com/)
 ...
 * @desc Creates a date picker associated with all elements with a class of
 *       "date-picker" then sets the selected date on these date pickers to the
 *       first Janurary 2010. When the date picker is next opened it will
 *       display Janurary 2010.
 */
 ...
 * the passed month is before startDate then the month containing startDate will
 * be displayed instead. If the passed month is after endDate then the month
 * containing the endDate will be displayed instead.
 * 
 * @param Number
 *            m The month you want the date picker to display. Optional -
 *            defaults to the currently displayed month.
 * @param Number
 *            y The year you want the date picker to display. Optional -
 *            defaults to the currently displayed year.
 * @type jQuery
 ...
 * date picker can be displayed at once then the date picker associated with the
 * last matched element will be the one that is displayed.
 * 
 * @param HTMLElement
 *            e An element that you want the date picker to pop up relative in
 *            position to. Optional - default behaviour is to pop up next to the
 *            element associated with this date picker.
 * @type jQuery
 * @name dpDisplay
 * @cat plugins/datePicker
 * @author Kelvin Luck (http://www.kelvinluck.com/)
 * 
 * @example $('#date-picker').datePicker(); $('#date-picker').dpDisplay();
 * @desc Creates a date picker associated with the element with an id of
 *       date-picker and then causes it to pop up.
 */
 ...
 *            a A function or an array of functions that are called when each td
 *            is rendered. Each function will receive four arguments; a jquery
 *            object wrapping the created TD, a Date object containing the date
 *            this TD represents, a number giving the currently rendered month
 *            and a number giving the currently rendered year.
 * @type jQuery
 ...
 * 
 * @example $('#date-picker').datePicker();
 *          $('#date-picker').dpSetRenderCallback(function($td, thisDate, month,
 *          year) { // do stuff as each td is rendered dependant on the date in
 *          the td and the displayed month and year });
 * @desc Creates a date picker associated with the element with an id of
 *       date-picker and then creates a function which is called as each td is
 *       rendered when this date picker is displayed.
 */
...
/**
 * Sets the position that the datePicker will pop up (relative to it's
 * associated element)
 * 
 * @param Number
 *            v The vertical alignment of the created date picker to it's
 *            associated element. Possible values are $.dpConst.POS_TOP and
 *            $.dpConst.POS_BOTTOM
 * @param Number
 *            h The horizontal alignment of the created date picker to it's
 *            associated element. Possible values are $.dpConst.POS_LEFT and
 *            $.dpConst.POS_RIGHT
 * @type jQuery
 * @name dpSetPosition
 * @cat plugins/datePicker
 ...
 * 
 * @example $('#date-picker').datePicker();
 *          $('#date-picker').dpSetPosition($.dpConst.POS_BOTTOM,
 *          $.dpConst.POS_RIGHT);
 * @desc Creates a date picker associated with the element with an id of
 *       date-picker and makes it so that when this date picker pops up it will
 *       be bottom and right aligned to the #date-picker element.
 */
...
/**
 * Sets the offset that the popped up date picker will have from it's default
 * position relative to it's associated element (as set by dpSetPosition)
 * 
 * @param Number
 *            v The vertical offset of the created date picker.
 * @param Number
 *            h The horizontal offset of the created date picker.
 * @type jQuery
 * @name dpSetOffset
 * @cat plugins/datePicker
 * @author Kelvin Luck (http://www.kelvinluck.com/)
 * 
 * @example $('#date-picker').datePicker(); $('#date-picker').dpSetOffset(-20,
 *          200);
 * @desc Creates a date picker associated with the element with an id of
 *       date-picker and makes it so that when this date picker pops up it will
 *       be 20 pixels above and 200 pixels to the right of it's default
 *       position.
 */
 ...
 * @author Kelvin Luck (http://www.kelvinluck.com/)
 * 
 * @example $('.date-pick') .datePicker() .bind( 'focus', function() {
 *          $(this).dpDisplay(); } ).bind( 'blur', function() {
 *          $(this).dpClose(); } );
 */
		dpClose : function()
		{
...
/**
 * Rerenders the date picker's current month (for use with inline calendars and
 * renderCallbacks).
 * 
 * @type jQuery
 ...
 */
		...
		// private function called on unload to clean up any expandos etc and
	...
	// private internal function to cut down on the amount of code needed where
	// we forward
	// dp* methods on the jQuery object on to the relevant DatePicker
	// controllers...
	...
	$.extend(
		...
		{	
			...
			setEndDate : function(d)
			{
				...
				if (!this.endDate) {
					this.endDate = (new Date('12/31/2999')); // using the JS
																// Date.parse
																// function
																// which expects
																// mm/dd/yyyy
			...
			setDisplayedMonth : function(m, y, rerender)
			{
				...
				// check if the desired date is within the range of our defined
			...
			setSelected : function(d, v, moveToMonth, dispatchEvents)
			{
				...
				if (s.selectWeek)
				{
					...
					if (d < this.startDate) // The first day of this week is
											// before the start date so is
											// unselectable...
					{
						return;
					}
				}
				if (v == this.isSelected(d)) // this date is already
												// un/selected
				{
					return;
				}
...
// $('.selectee', this.context).focus();
			...
			cellRender : function ($td, thisDate, month, year) {
				...
				// add our click handlers to deal with it when the days are
				// clicked...
			...
			// ele is the clicked button - only proceed if it doesn't have the
			// class disabled...
			// m and y are -1, 0 or 1 depending which direction we want to go
			// in...
			...
			_renderCalendar : function()
			{
				...
				// update the status of the control buttons and disable dates
				// before startDate or after endDate...
				// TODO: When should the year buttons be disabled? When you
				// can't go forward a whole year from where you are or is that
				// annoying?
				if (this.displayedYear == this.startDate.getFullYear() && this.displayedMonth == this.startDate.getMonth()) {
					...
					var d = this.startDate.getDate();
					if (d > 20) {
						// check if the startDate is last month as we might need
						// to add some disabled classes...
				...
				if (this.displayedYear == this.endDate.getFullYear() && this.displayedMonth == this.endDate.getMonth()) {
					...
					var d = this.endDate.getDate();
					if (d < 13) {
						// check if the endDate is next month as we might need
						// to add some disabled classes...
			...
			// empties the current dp-calendar div and makes sure that all
			// events are unbound
	...
	// comments to only include bgIframe where it is needed in IE without
...
/*
 * timeago: a jQuery plugin, version: 0.9.2 (2010-09-14) @requires jQuery v1.2.3
 * or later
 * 
...
(function($, window) {
  ...
  // The HashLocationProxy is the default location proxy for all Sammy
	// applications.
  // A location proxy is a prototype that conforms to a simple interface. The
	// purpose
  // of a location proxy is to notify the Sammy.Application its bound to when
	// the location
  // or 'external state' changes. The HashLocationProxy considers the state to
	// be
  // changed when the 'hash' (window.location.hash / '#') changes. It does
	// this in two
  // different ways depending on what browser you are using. The newest
	// browsers
  // (IE, Safari > 4, FF >= 3.6) support a 'onhashchange' DOM event, thats
	// fired whenever
  // the location.hash changes. In this situation the HashLocationProxy just
	// binds
  // to this event and delegates it to the application. In the case of older
	// browsers
  // a poller is set up to track changes to the hash. Unlike Sammy 0.3 or
	// earlier,
  // the HashLocationProxy allows the poller to be a global object,
	// eliminating the
  ...
  // The only argument an 'app_function' is evaluated within the context of
	// the application.
  ...
  Sammy.Application.prototype = $.extend({}, Sammy.Object.prototype, {
	...
	// function
    // that takes two arguments, the content of the unrendered partial and an
	// optional
    // JS object that contains interpolation data. Template engine is only
	// called/refered
    ...
    // location mechanism. See `Sammy.HashLocationProxy` and
	...
	// http://code.quirkey.com/sammy/doc/routes.html
    //
    ...
    // ### Arguments
    //
    // * `verb` A String in the set of ROUTE_VERBS or 'any'. 'any' will add
	// routes for each
    // of the ROUTE_VERBS. If only two arguments are passed,
    // the first argument is the path, the second is the callback and the verb
    // is assumed to be 'any'.
    // * `path` A Regexp or a String representing the path to match to invoke
	// this verb.
    // * `callback` A Function that is called/evaluated whent the route is run
	// see: `runRoute()`.
    // It is also possible to pass a string as the callback, which is looked up
	// as the name
    ...
    route: function(verb, path, callback) {
      ...
      if (path.constructor == String) {
        ...
        // Needs to be explicitly set because IE will maintain the index unless
		// NULL is returned,
        // which means that with two consecutive routes that contain params, the
		// second set of params will not be found and end up in splat instead of
		// params
    ...
    // as arguments, this allows for mass definition of routes. Another benefit
	// is
    ...
    // ### Arguments
    //
    // * `name` The name of the event. Automatically prefixed with the
	// `eventNamespace()`
    // * `data` An optional Object that can be passed to the bound callback.
    // * `context` An optional context/Object in which to execute the bound
	// callback.
    ...
    // more usefuly wrapping callback inside the result of an asynchronous
	// execution.
    //
    // ### Example
    //
    // The most common use case for around() is calling a _possibly_ async
	// function
    ...
    // a single method at a time. This is especially useful for dynamically
	// named
    // helpers
    //
    // ### Example
    ...
    // within a document.ready block to ensure the DOM exists before binding
	// events, etc.
    //
    // ### Example
    //
    ...
    // ### Arguments
    //
    // * `start_url` Optionally, a String can be passed which the App will
	// redirect to
    ...
    // Given a verb and a String path, will return either a route object or
	// false
    ...
    // ### Returns
    //
    // Either returns the value returned by the route callback or raises a 404
	// Not Found error.
    //
    ...
    // object to _only_ match certain paths or verbs, or match all paths or
	// verbs _except_
    ...
    // this will re-throw the error to the browser. Otherwise it will send the
	// error
  ...
  // `Sammy.RenderContext` is an object that makes sequential template
	// loading,
  ...
  //
  // The core magic is in the `then()` method which puts the callback passed
	// as
  // an argument into a queue to be executed once the previous callback is
	// complete.
  // All the methods of `RenderContext` are wrapped in `then()` which allows
	// you
  // to queue up methods by chaining, but maintaing a guarunteed execution
	// order
  ...
  Sammy.RenderContext.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    // queue. If the context is `waiting` (meaning an async operation is
	// happening)
    // then the callback will be executed in order, once the other operations
	// are
    ...
    //
    // If a jQuery or DOM object is passed the `innerHTML` of the node is pulled
	// in.
    // This is useful for nesting templates as part of the initial page load
	// wrapped
    // in invisible elements or `<script>` tags. With template paths, the
	// template
    ...
    // callback takes the same style of arguments as `jQuery.each()` (index,
	// item).
    // The return value of each callback is collected as a single string and
	// stored
  ...
  // bound event is triggered. The callbacks for these events are evaluated
	// within a `Sammy.EventContext`
  // This within these callbacks the special methods of `EventContext` are
	// available.
  //
  // ### Example
  //
  ...
  // * `params` An Object of optional params to pass to the context. Is
	// converted
  // to a `Sammy.Object`.
  // * `target` a DOM element that the event that holds this context
	// originates
  // from. For post, put and del routes, this is the form element that
	// triggered
  // the route.
  //
  ...
  Sammy.EventContext.prototype = $.extend({}, Sammy.Object.prototype, {
    ...
    // // sets the `content` to <div class="name">quirkey</div><div
	// class="name">endor</div>
    // renderEach('mytemplate.mustache', [{name: 'quirkey'}, {name:
	// 'endor'}]).appendTo('ul');
    // // appends the rendered content to $('ul')
    //
    renderEach: function(location, name, data, callback) {
